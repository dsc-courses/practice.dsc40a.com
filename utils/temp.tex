...
} $\times$ ( ) $\vec{c}$} \bubble{$\vec{c_f}$} \bubble[fill=black!90]{$\vec{d}$} \bubble{$\vec{d_f}$
...
\inlineresponsebox[1.5in]{1in} $\times$ ( ) $\vec{c}$} \bubble{$\vec{c_f}$} \bubble{$\vec{d}$} \bubble{$\vec{d_f}$
...
( ) $7 < h < 9$} \bubble{$7 \leq h < 9$} \bubble{$7 < h \leq 9$} \bubble{$7 \leq h \leq 9$
