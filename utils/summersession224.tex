...
} $\times$ \bubble{$\vec{c}$} \bubble{$\vec{c_f}$} \bubble[fill=black!90]{$\vec{d}$} \bubble{$\vec{d_f}$}

\end{subprob}

\begin{subprob}(3 pts) The projection of $\vec{c}$ onto $\text{span}(\vec{d})$.

\inlineresponsebox[1.5in]{1in} $\times$ \bubble{$\vec{c}$} \bubble{$\vec{c_f}$} \bubble{$\vec{d}$} \bubble{$\vec{d_f}$}
...
\bubble{The maximum of $y_1, y_2, ..., y_n$}

\bubble{The mean of $y_1, y_2, ..., y_{n-1}$}

\bubble{The mean of $y_2, y_3, ..., y_n$}

\bubble{The mean of $y_2, y_3, ..., y_n$, multiplied by $\frac{n}{n-1}$}
...
\begin{enumerate}[label=(\roman*)]
    ...
    \bubble{none} \bubble{1} \bubble{2} \bubble{3} \bubble{all 4}
\end{enumerate}
